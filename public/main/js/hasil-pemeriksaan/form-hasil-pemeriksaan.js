...
$(document).ready(function () {
  ...
  $("#btnSubmitHasilPemeriksaan").click(function () {
    const getCheckedStatusPemeriksa = $(
      "input[name='radioStatusPemeriksa']:checked"
    ).val();

    if (parseInt(getCheckedStatusPemeriksa)) {
      $("#modal-confirmation .modal-title").text("Peringatan");
      $("#modal-confirmation .box-body")
        .text(`Anda yakin ingin menyelesaikan Hasil Pemeriksaan ini? Jika menyelesaikan Hasil Pemeriksaan ini anda
  ...
  $("#selectedPasien").on("select2:select", function (e) {
    ...
    if (getObj) {
      $("#nomorPasienTxt").text(getObj.id_number_patient);
      $("#jenisHewanTxt").text(getObj.pet_category);
      $("#namaHewanTxt").text(getObj.pet_name);
      $("#jenisKelaminTxt").text(getObj.pet_gender);
      $("#usiaHewanTahunTxt").text(`${getObj.pet_year_age} Tahun`);
      $("#usiaHewanBulanTxt").text(`${getObj.pet_month_age} Bulan`);
      $("#namaPemilikTxt").text(getObj.owner_name);
      $("#alamatPemilikTxt").text(getObj.owner_address);
      $("#nomorHpPemilikTxt").text(getObj.owner_phone_number);
      $("#nomorRegistrasiTxt").text(getObj.registration_number);
      $("#keluhanTxt").text(getObj.complaint);
      $("#namaPendaftarTxt").text(getObj.registrant);
    } else {
      refreshText();
    }
  ...
  $("#descriptionCondPasien").keyup(function () {
    validationForm();
  });

  $("#selectedJasa").on("select2:select", function (e) {
    processSelectedJasa(e.params.data.id, e.params.data.selected);
    validationForm();
  });
  ...
  function processSaved() {
    ...
    fd.append(
      "status_finish",
      $("input[name='radioStatusPemeriksa']:checked").val()
    );
    fd.append(
      "status_outpatient_inpatient",
      $("input[name='radioRawatInap']:checked").val()
    );
    ...
    selectedListJasa.forEach((lj) => {
      finalSelectedJasa.push({
        price_service_id: lj.price_service_id,
        quantity: lj.quantity,
        price_overall: lj.price_overall,
      });
    });
    ...
    arrayKelompokObat.forEach((ko) => {
      ...
      ko.selectedListBarang.forEach((lb) => {
        newObj.list_of_medicine.push({
          price_item_id: lb.price_item_id,
          quantity: lb.quantity,
          price_overall: lb.price_overall,
        });
      });
  ...
  function processEdit() {
    ...
    deletedUpdateListJasa.forEach((ulj) => {
      finalSelectedJasa.push({
        id: ulj.id,
        price_service_id: ulj.price_service_id,
        quantity: ulj.quantity,
        price_overall: ulj.price_overall,
        status: "del",
      });
    });
    ...
    arrayKelompokObatDelete.forEach((ko) => {
      ...
      ko.deletedUpdateListBarang.forEach((dul) => {
        newObj.list_of_medicine.push({
          id: dul.id,
          price_item_id: dul.price_item_id,
          quantity: dul.quantity,
          price_overall: dul.price_overall,
          status: "del",
        });
      });
    ...
    const datas = {
      ...
      status_finish: parseInt(
        $("input[name='radioStatusPemeriksa']:checked").val()
      ),
      status_outpatient_inpatient: parseInt(
        $("input[name='radioRawatInap']:checked").val()
      ),
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        $.ajax({
          url:
            $(".baseUrl").val() + "/api/hasil-pemeriksaan/update-upload-gambar",
          type: "POST",
          dataType: "JSON",
          headers: { Authorization: `Bearer ${token}` },
          data: fdUpload,
          ...
          error: function (err) {
            if (err.status === 422) {
              ...
              $.each(err.responseJSON.errors, function (idx, v) {
                errText +=
                  v +
                  (idx !== err.responseJSON.errors.length - 1 ? "<br/>" : "");
              });
              $("#beErr").append(errText);
              isBeErr = true;
            } else if (err.status == 401) {
  ...
  function processSelectedJasa(selectedId, selected) {
    if (selected) {
      ...
      const getIds = [];
      const getIdx = selectedListJasa.findIndex(
        (i) => i.price_service_id == selectedId
      );

      deletedUpdateListJasa.push(selectedListJasa[getIdx]);
      selectedListJasa.splice(getIdx, 1);
      selectedListJasa.forEach((lj) => {
        getIds.push(lj.price_service_id);
      });
      if (!selectedListJasa.length) {
        $(".table-list-jasa").hide();
      }

      $("#selectedJasa").val(getIds);
      $("#selectedJasa").trigger("change");
      processAppendListSelectedJasa();
    }
  }
  ...
  function processAppendListSelectedJasa() {
    ...
    selectedListJasa.forEach((lj, idx) => {
      rowSelectedListJasa +=
        ...
        `${
          formState == "edit"
            ? "<td>" + (lj.created_at ? lj.created_at : "-") + "</td>"
            : ""
        }` +
        `${
          formState == "edit"
            ? "<td>" + (lj.created_by ? lj.created_by : "-") + "</td>"
            : ""
        }` +
        ...
        `<td>${
          typeof lj.selling_price == "string"
            ? lj.selling_price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")
            : ""
        }</td>` +
  ...
  function loadFormEdit() {
    ...
    $('input[name="radioRawatInap"]').prop("disabled", true);
    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        getId = getData.id;
        getPatienRegistrationId = getData.patient_registration_id;
        $("#nomorRegistrasiTxt").text(getData.registration.registration_number);
        $("#nomorPasienTxt").text(getData.registration.patient_number);
        $("#jenisHewanTxt").text(getData.registration.pet_category);
        $("#namaHewanTxt").text(getData.registration.pet_name);
        $("#jenisKelaminTxt").text(getData.registration.pet_gender);
        $("#usiaHewanTahunTxt").text(
          `${getData.registration.pet_year_age} Tahun`
        );
        $("#usiaHewanBulanTxt").text(
          `${getData.registration.pet_month_age} Bulan`
        );
        $("#namaPemilikTxt").text(getData.registration.owner_name);
        $("#alamatPemilikTxt").text(getData.registration.owner_address);
        $("#nomorHpPemilikTxt").text(getData.registration.owner_phone_number);
        $("#keluhanTxt").text(getData.registration.complaint);
        $("#namaPendaftarTxt").text(getData.registration.registrant);

        $("#anamnesa").val(getData.anamnesa);
        $("#diagnosa").val(getData.diagnosa);
        ...
        if (getData.services.length) {
          getData.services.forEach((item) => {
            selectedListJasa.push({
              ...
              status_paid_off: item.status_paid_off,
              quantity: item.quantity,
              price_overall: item.price_overall,
        ...
        if (getData.item.length) {
          getData.item.forEach((item) => {
            let newObj = {
              id: null,
              kelompokObatId: null,
              selectDropdownBarang: [],
              selectedListBarang: [],
              deletedUpdateListBarang: [],
            };
            ...
            item.list_of_medicine.forEach((lom) => {
              ...
              newObj.selectedListBarang.push({
                ...
                item_name: lom.item_name,
                unit_name: lom.unit_name,
                selling_price: lom.selling_price,
                status_paid_off: lom.status_paid_off,
                quantity: lom.quantity,
                price_overall: lom.price_overall,
                created_at: lom.created_at,
                created_by: lom.created_by,
              });
            });
        ...
        $(
          `input[name=radioRawatInap][value=${getData.status_outpatient_inpatient}]`
        ).prop("checked", true);
        ...
        $(
          `input[name=radioStatusPemeriksa][value=${getData.status_finish}]`
        ).prop("checked", true);
        ...
        if (getData.images.length) {
          getData.images.forEach((img, i) => {
            ...
            $(`.box-image-upload img.img-preview-${i + 1}`).attr(
              "src",
              $(".baseUrl").val() + img.image
            );
            $(`.box-image-upload a.img-preview-${i + 1}`).attr(
              "href",
              $(".baseUrl").val() + img.image
            );
            $(`.box-image-upload a.img-preview-${i + 1}`).attr(
            ...
            $(
              `.box-image-upload img.img-preview-${
                i + 1
              }, .box-image-upload a.img-preview-${i + 1}`
            ).show();
  ...
  function formConfigure() {
    ...
    $("#selectedJasa").select2({
      placeholder: "Jenis Pelayanan - Kategori Jasa",
      allowClear: true,
    });
  ...
  function refreshText() {
    ...
    $("#jenisHewanTxt").text("-");
    $("#namaHewanTxt").text("-");
    $("#jenisKelaminTxt").text("-");
    $("#usiaHewanTahunTxt").text("- Tahun");
    $("#usiaHewanBulanTxt").text("- Bulan");
    $("#namaPemilikTxt").text("-");
    $("#alamatPemilikTxt").text("-");
    $("#nomorHpPemilikTxt").text("-");
    $("#nomorRegistrasiTxt").text("-");
    $("#keluhanTxt").text("-");
    $("#namaPendaftarTxt").text("-");
  }
...
function validationForm() {
  if (formState === "add") {
    if (!$("#selectedPasien").val()) {
      $("#pasienErr1").text("Pasien harus di isi");
      isValidSelectedPasien = false;
    } else {
      $("#pasienErr1").text("");
      isValidSelectedPasien = true;
    }
  } else {
    isValidSelectedPasien = true;
  }
  ...
  if (!$("#anamnesa").val()) {
    $("#anamnesaErr1").text("Anamnesa harus di isi");
    isValidAnamnesa = false;
  } else {
    $("#anamnesaErr1").text("");
    isValidAnamnesa = true;
  }

  if (!$("#sign").val()) {
    $("#signErr1").text("Sign harus di isi");
    isValidSign = false;
  } else {
    $("#signErr1").text("");
    isValidSign = true;
  ...
  if (!$("#diagnosa").val()) {
    $("#diagnosaErr1").text("Diagnosa harus di isi");
    isValidDiagnosa = false;
  } else {
    $("#diagnosaErr1").text("");
    isValidDiagnosa = true;
  }

  if (!$("input[name='radioRawatInap']:checked").val()) {
    $("#rawatInapErr1").text("Rawat inap harus di isi");
    isValidRadioRawatInap = false;
  } else {
    $("#rawatInapErr1").text("");
    isValidRadioRawatInap = true;
  }

  if (!$("input[name='radioStatusPemeriksa']:checked").val()) {
    $("#statusPemeriksaErr1").text("Status Pemeriksa harus di isi");
    isValidRadioStatusPemeriksa = false;
  } else {
    $("#statusPemeriksaErr1").text("");
    isValidRadioStatusPemeriksa = true;
  }

  let isValidKelompokObat = true;
  for (let i = 0; i < arrayKelompokObat.length; i++) {
    if (
      !arrayKelompokObat[i].kelompokObatId ||
      !arrayKelompokObat[i].selectedListBarang.length
    ) {
      $("#customErr1").text(
        "Terdapat Kelompok Obat atau Daftar Barang yang belum diisi, harap dicek kembali!"
      );
      customErr1 = true;
      isValidKelompokObat = false;
      break;
    }
  }
  ...
  if (
    !isValidSelectedPasien ||
    !isValidAnamnesa ||
    !isValidSign ||
    !isValidDiagnosa ||
    !isValidRadioRawatInap ||
    !isValidRadioStatusPemeriksa ||
    !isValidFotoKondisiPasien ||
    isBeErr ||
    customErr1
  ) {
    $("#btnSubmitHasilPemeriksaan").attr("disabled", true);
