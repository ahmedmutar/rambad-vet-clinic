...
function drawListKelompokObat() {
  ...
  arrayKelompokObat.forEach((ko, idx) => {
    let rowSelectedListBarang = appendListSelectBarang(
      ko.selectedListBarang,
      idx
    );
    ...
    rowKelompokObat +=
      ...
      `</div>` +
      `<div class="table-responsive" id="table-list-barang-${idx}" style="display: ${
        ko.selectedListBarang.length ? "block" : "none"
      }">` +
      `<table class="table table-striped text-nowrap">` +
      `<thead>` +
      `<tr>` +
      `<th>No</th>` +
      `<th class="tgl-edit" style="display: ${
        formState == "edit" ? "" : "none"
      }">Tanggal</th>` +
      `<th class="dibuat-edit" style="display: ${
        formState == "edit" ? "" : "none"
      }">Dibuat Oleh</th>` +
      `<th>Nama Barang</th>` +
  ...
  $(".list-selected-barang").on("input", ".qty-input-barang", function () {
    ...
    arrayKelompokObat[idxKelompokObat].selectedListBarang[idx].price_overall =
      totalBarang;
    validationForm();
  ...
  $(`.list-selected-barang`).on(
    "click",
    "button.btnRemoveSelectedListBarang",
    function (e) {
      const getIds = [];
      const idx = $(this).attr("index");
      const idxKelompokObat = $(this).attr("idxKelompokObat");

      arrayKelompokObat[idxKelompokObat].deletedUpdateListBarang.push(
        arrayKelompokObat[idxKelompokObat].selectedListBarang[idx]
      );
      arrayKelompokObat[idxKelompokObat].selectedListBarang.splice(idx, 1);
      arrayKelompokObat[idxKelompokObat].selectedListBarang.forEach((lb) => {
        getIds.push(lb.price_item_id);
      });
      if (!arrayKelompokObat[idxKelompokObat].selectedListBarang.length) {
        $(`#table-list-barang-${idxKelompokObat}`).hide();
      }
      arrayKelompokObat[idxKelompokObat].selectDropdownBarang.splice(idx, 1);

      validationForm();
      $(`#selectedBarang-${idxKelompokObat}`).val(getIds);
      $(`#selectedBarang-${idxKelompokObat}`).trigger("change");
  ...
  $(".selectedKelompokObat").each(function (index, obj) {
    ...
    if (listKelompokObat.length) {
      for (let i = 0; i < listKelompokObat.length; i++) {
        optKelompokObat += `<option value=${listKelompokObat[i].id}>${
          listKelompokObat[i].group_name
        } - ${listKelompokObat[i].branch_name} - ${listKelompokObat[
          i
        ].selling_price
          .toString()
...
function appendDropdownSelectBarang() {
  $(".selectedBarang").each(function (index, obj) {
    ...
    $(this)
      .select2({
        placeholder: "Nama Barang - Kategori Barang",
        allowClear: true,
      })
      .val(getValue)
      .trigger("change");
    ...
    rowSelectedListBarang +=
      ...
      `<td>${
        typeof lb.selling_price == "string"
          ? lb.selling_price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")
          : ""
      }</td>` +
      ...
      `<td>
          <button type="button" class="btn btn-danger btnRemoveSelectedListBarang"
            ${
              lb.status_paid_off ? "disabled" : ""
            } idxKelompokObat=${idxKelompokObat} index=${idx}><i class="fa fa-trash-o" aria-hidden="true"></i></button>
        </td>` +
      `</tr>`;
    ++no;
  });
...
function processSelectedBarang(selectedId, selected, idx) {
  ...
  if (selected) {
    ...
    const getIds = [];
    const getIdx1 = arrayKelompokObat[idx].selectedListBarang.findIndex(
      (i) => i.price_item_id == selectedId
    );
    const getIdx2 = arrayKelompokObat[idx].selectDropdownBarang.findIndex(
      (i) => i == selectedId
    );

    arrayKelompokObat[idx].selectedListBarang.splice(getIdx1, 1);
    arrayKelompokObat[idx].deletedUpdateListBarang.push(
      arrayKelompokObat[idx].selectedListBarang[getIdx1]
    );

    arrayKelompokObat[idx].selectDropdownBarang.splice(getIdx2, 1);

    arrayKelompokObat[idx].selectedListBarang.forEach((lb) => {
      getIds.push(lb.price_item_id);
    });
    if (!arrayKelompokObat[idx].selectedListBarang.length) {
      $("#table-list-barang-" + idx).hide();
    }
