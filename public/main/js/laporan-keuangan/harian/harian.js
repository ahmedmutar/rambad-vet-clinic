$(document).ready(function () {
  ...
  $(".btn-download-excel").click(function () {
    const getBranchId =
      role.toLowerCase() == "dokter" ? branchId : paramUrlSetup.branchId;
  ...
  function loadLaporanKeuanganHarian() {
    $.ajax({
      ...
      success: function (resp) {
        ...
        if (getData.length) {
          $.each(getData, function (idx, v) {
            listLaporanKeuanganHarian +=
              ...
              `<td>${v.complaint}</td>` +
              `<td>${
                v.status_outpatient_inpatient == 1
                  ? "Rawat Inap"
                  : "Rawat Jalan"
              }</td>` +
              `<td>${
                typeof v.price_overall == "string"
                  ? v.price_overall
                      .toString()
                      .replace(/\B(?=(\d{3})+(?!\d))/g, ".")
                  : ""
          ...
          listLaporanKeuanganHarian += `<tr class="text-center"><td colspan="14">Tidak ada data.</td></tr>`;
        }
        $("#list-laporan-keuangan-harian").append(listLaporanKeuanganHarian);

        const priceOverall =
          resp.price_overall > -1
            ? resp.price_overall
                .toString()
                .replace(/\B(?=(\d{3})+(?!\d))/g, ".")
            : "-";
        const capitalPrice =
          resp.capital_price > -1
            ? resp.capital_price
                .toString()
                .replace(/\B(?=(\d{3})+(?!\d))/g, ".")
            : "-";
        const docterFee =
          resp.doctor_fee > -1
            ? resp.doctor_fee.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")
            : "-";
        const petshopFee =
          resp.petshop_fee > -1
            ? resp.petshop_fee.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ".")
            : "-";
        ...
        $(".openDetail").click(function () {
          window.location.href =
            $(".baseUrl").val() +
            `/laporan-keuangan-harian/detail/${$(this).val()}?date=${
              paramUrlSetup.date
            }`;
        });
      },
