$(document).ready(function() {
  ...
  $('.btn-back-to-list .text, #btnKembali').click(function() {
    window.location.href = $('.baseUrl').val() + '/petshop';
  });
  ...
  $('#btnSubmitPetshop').click(function() {

    $('#modal-confirmation .modal-title').text('Peringatan');
    $('#modal-confirmation .box-body').text('Anda yakin ingin merubah Barang ini? Data yang akan anda tambahkan tidak dapat diubah kembali.');
    $('#modal-confirmation').modal('show');
  });


  $.ajax({
    url     : $('.baseUrl').val() + '/api/petshop/detail',
  ...
  function processAppendListSelectedJasa() {
    ...
    selectedListJasa.forEach((lj, idx) => {
      rowSelectedListJasa += `<tr>`
        ...
        + `<td>
              <button type="button" class="btn btn-danger cancelPetshopJasa ${lj.status_paid_off && lj.isRevert ? 'd-none':'d-block'}" title="Membatalkan petshop"
              ${role.toLowerCase() != 'admin' || !lj.status_paid_off ? 'disabled' : ''} index=${idx}><i class="fa fa-close" aria-hidden="true"></i></button>
              <button type="button" class="btn btn-success revertPetshopJasa
    ...
    $('.cancelPetshopJasa').click(function() {
    ...
    $('.revertPetshopJasa').click(function() {
      getIdx = $(this).attr('index');
  ...
  function processAppendListSelectedBarang() {
    ...
    selectedListBarang.forEach((lb, idx) => {
      rowSelectedListBarang += `<tr>`
        ...
        + `<td>
            <button type="button" class="btn btn-danger cancelPetshopBarang ${lb.status_paid_off && lb.isRevert ? 'd-none':'d-block'}" title="Membatalkan petshop"
            ${role.toLowerCase() != 'admin' || !lb.status_paid_off ? 'disabled' : ''} index=${idx}><i class="fa fa-close" aria-hidden="true"></i></button>
            <button type="button" class="btn btn-success revertPetshopBarang
    ...
    $('.cancelPetshopBarang').click(function() {
      getIdx = $(this).attr('index');
    ...
    $('.revertPetshopBarang').click(function() {
      getIdx = $(this).attr('index');
  ...
  function processSaved() {
    ...
    $.ajax({
      ...
      , error: function(err) {
        if (err.status === 422) {
          let errText = ''; $('#beErr').empty(); $('#btnSubmitPetshop').attr('disabled', true);
  ...
  function validationForm() {
    ...
    $('#btnSubmitPetshop').attr('disabled', (!isValidCalculationPay || isBeErr) ? true : false);
  }

  function processPrint(check_up_result_id, service_payment, item_payment) {
    let url = '/petshop/print/' + check_up_result_id + '/' + service_payment + '/' + item_payment;
