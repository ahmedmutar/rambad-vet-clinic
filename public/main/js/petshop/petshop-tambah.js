$(document).ready(function () {
  ...
  $('#selectedPasien').on('select2:select', function (e) {
    refreshVariableTambahPetshop();

    if ($(this).val()) {
      $.ajax({
        url: $('.baseUrl').val() + '/api/hasil-pemeriksaan/petshop',
  ...
  $('#btnSubmitPetshop').click(function () {
    ...
    $('#modal-confirmation .modal-title').text('Peringatan');
    $('#modal-confirmation .box-body').text('Anda yakin ingin menyimpan barang ini? Data yang akan anda tambahkan tidak dapat diubah kembali.');
  ...
  function loadPasien() {
    $.ajax({
      url: $('.baseUrl').val() + '/api/petshop/pasien',
      headers: { 'Authorization': `Bearer ${token}` },
      type: 'GET',
      beforeSend: function () { $('#loading-screen').show(); },
      success: function (data) {
  ...
  function validationForm() {
    ...
    $('#btnSubmitPetshop').attr('disabled', (!isValidSelectedPasien || !isValidCalculationPay || isBeErr) ? true : false);
  }
  ...
  function refreshVariableTambahPetshop() {
